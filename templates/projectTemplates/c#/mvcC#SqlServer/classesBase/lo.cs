...
*/
    ...
    public static void tratarError(Exception exc, string descripcion, string datos)
    {
        ...
        if (ctes.mostrarErrores)
        {
            ...
            if (bot==false)
            {
                lo.enviarCorreoError(ctes.direccionCorreoErroresDestino, ctes.direccionCorreoErroresRemitente, lo.construirMensajeError(exc, datos), ctes.nombreAplicacion + " - error controlado. " + descripcion);
            }
            string pat = HttpContext.Current.Request.Url.Authority.ToString();

            HttpContext.Current.Response.Write("jkjkj");
            //HttpContext.Current.Response.Redirect("http://" + pat + "/error_udp.aspx");
    ...
    public static void enviarCorreo(string destinatario, string remitente, string texto, string asunto, string archivoAdjunto)
    {
        ...
        try
        {
            ...
            correo.From = new System.Net.Mail.MailAddress("correo@mayoresudp.net");
            ...
            System.Net.NetworkCredential basicAuthenticationInfo = new NetworkCredential("ace528c", "Correo01");

            Smtpx.Host = ctes.servidorSmtpErrores;
